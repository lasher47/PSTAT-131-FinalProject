...
Our accuracy for this type of model peaks around 0.729. 
...
```{r, eval = FALSE}
save(log_wkflow, file = "results/log_wkflow.rda")
```


```{r, warning = FALSE}
load(file = "results/log_wkflow.rda")
...
```
Our testing accuracy is about 0.722 which is about the same value that we obtained from the training data.
