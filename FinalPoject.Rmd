...
league_clean <- league_clean %>%
  select(-c(redFirstBlood, redExperienceDiff, blueExperienceDiff, redGoldDiff, blueGoldDiff)) #Redundant difference variables
...
ggplot(league_clean, aes(x=blueHeralds, y=blueTowersDestroyed)) +  geom_bar(fill='blue')

x <- league_clean %>%
  group_by(league_clean$blueHeralds) %>%
  summary(league_clean$blueTowersDestroyed)
