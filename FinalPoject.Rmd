...
We'll start by setting up an initial recipe and workflow below. We'll use every predictor left in our cleaned dataset as predictors with the exception of **gameID** and **blueWins**. Interactions terms are also included with reasoning supported from our EDA. 
...
Logistic Regression done below with tuning parameters  evaluated over our folds. We will 
...
```{r rf, eval = FALSE}
#TEMP EVAL=FALSE, FIX THIS LATER MAYBE
...
rf_param_grid <- grid_regular(mtry(range=c(1,15)), trees(range=c(64,128)), min_n(range = c(50, 500)), levels = 3)
